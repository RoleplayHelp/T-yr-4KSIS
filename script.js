...
function calculateDamage() {
    ...
    if (attackerElement === 'normal') {
        if (hpPercentage <= 25) baseDamage *= 1.20;
        else if (hpPercentage <= 50) baseDamage *= 1.15;
        else if (hpPercentage <= 75) baseDamage *= 1.10;
        else if (hpPercentage < 100) baseDamage *= 1.05;
    ...
    let finalDamage = Math.max(0, baseDamage - effectiveDef);
    ...
    displayResults(finalDamage, hitRate, dotDamage, {
        atk, dmgBuff, penetration, weaponType, rangedWeaponType, attackerElement, def, currentHP, maxHP, hasBlock, distance, defenderElement, defReduction, effectiveDef, hpPercentage, damageMultiplier
    });
}
...
function displayResults(damage, hitRate, dotDamage, params) {
    ...
    output += `5. Phòng thủ hiệu quả = DEF * (1 - defReduction) * (2 nếu block) * (1 - penetration) = ${params.def} * (1 - ${params.defReduction}) * ${params.hasBlock ? 2 : 1} * (1 - ${params.penetration}) = ${params.effectiveDef.toFixed(2)}\n`;
    output += `6. Sát thương cuối cùng = Max(0, Sát thương - Phòng thủ hiệu quả) = ${damage.toFixed(2)}`;
    output += '</pre>';
    ...
    content.innerHTML = `
        ...
        <ul style="color: #2c3e50;">
            ...
            <li><strong style="color: #9b59b6;">Hiệu ứng:</strong>
                <ul style="color: #34495e;">
					<li><span style="color: #9b59b6;">Đây là các hiệu ứng mecha đang có trên người, nếu không có hãy bỏ qua</span></li>
